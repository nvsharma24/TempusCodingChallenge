...
annotatevcf <- function(fn){
  ...
  annodata <- cbind(chr, start, end, ref, alt, myinfo[,"DP"], perc_alt_reads, perc_ref_reads,myinfo[,"TYPE"])
  exacq <- paste(annodata[,1],"-", annodata[,2], "-", annodata[,4], "-", annodata[,5], sep = "")
  annodata <- cbind(annodata, exacq)
  colnames(annodata) <- c("chr", "start", "end", "ref_allele", "alt_allele", "read_depth", "perc_alt-reads", "perc_ref-reads", "var_type", "exacq")
